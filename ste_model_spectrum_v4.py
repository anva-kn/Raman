...
def find_mul_peak_sym_supp(int_idx, y_data_win, mean_level_win, discount=0.85):
    ...
    # win_left=np.mean(Y_smooth[:l_pos_temp-1])*discount
    # win_right=np.mean(Y_smooth[r_pos_temp+1:])*discount
    
    
    # while win_left<=Y_smooth[l_pos_temp]  and  l_pos_temp>0:        
    #     l_pos_temp=l_pos_temp-1
    #     win_left=np.mean(Y_smooth[:l_pos_temp])*discount
    
    # # then go right    
    # while  win_right<=Y_smooth[r_pos_temp] and r_pos_temp<res-1:     
    #     r_pos_temp=r_pos_temp+1
...
def find_win_supp(peak,l_pos,r_pos, y_up):
    ...
    # win_left=np.mean(Y_smooth[:l_pos_temp-1])
    # win_right=np.mean(Y_smooth[r_pos_temp+1: ])
    
    # UPDATE THIS: TAKE THE MEAN ON BOTH SIDES
...
def identify_fitting_win_up(f_sup,space_mean,slide_win=100,num_th=10,fit_fun=lor_amp, init_fit_fun=init_lor,peak_tol=0.85):
    ...
    # data_mean=np.meandiscount(data[2],axis=1)
    ...
    idx_pos_min=np.concatenate(( [0]   , np.argsort((y_data-poly_interp)**2)[:num_th], [x_data.shape[0]-1]))
    
    mean_level  = interp1d(x_data[idx_pos_min],y_data[idx_pos_min])(x_data)
    
    # update the interpolation as you identify peaks
    th_vec=np.linspace(-np.sort(-(y_data-mean_level))[win_small],0,num_th+1)
    
    fitting_data=np.empty(num_th,dtype=object)
    
    for j in  range(th_vec.shape[0]-1):
            ...
            pos=np.array(np.where((y_data-mean_level)<th_vec[j])[0])
            
            if False:
                plt.plot(x_data,y_data)
                plt.plot(x_data[pos],y_data[pos],'*')
            ...
            idx_lr=idx_lr[np.where(idx_lr[:,1]-idx_lr[:,0]>1)[0],:] 
            ...
            poly_interp=np.poly1d(np.polyfit(x_data[ind_poly],y_data[ind_poly],4))(x_data)
            idx_pos_min=np.concatenate(( [0]   , np.argsort((y_data-poly_interp)**2)[:num_th], [x_data.shape[0]-1]))
            ...
            #-------------------------------------------------------------------
            # Recursive fitting of the window up
            #-------------------------------------------------------------------
            ...
            if idx_lr_comp.size>0:
                for k in range(idx_lr_comp.shape[0]):
                        ...
                        ##### remove the peaks and leave the poly
                        idx_comp_lr=find_mul_peak_sym_supp(idx_lr_comp[k],y_data,mean_level,0.85)
                        idx_comp = range(idx_comp_lr[0],idx_comp_lr[1]+1)
                        ...
                        recursive_merge_up(x_rec, y_rec, mean_rec, th_vec[j], mse_comp, range_comp, beta_comp, fit_fun, init_fit_fun)
            ...
            if range_p.size==0:
                ...
                fitting_data[j]=dict( idx_lr_poly = idx_lr_poly, lin_interp=mean_level, range_peak = 0 )
            else:
                print('Storing Th',j,'Range',range_p[:,0],'--',range_p[:,1])
                fitting_data[j]=dict( idx_lr_poly = idx_lr_poly , lin_interp=mean_level, mse_peak = mse_comp , range_peak = range_comp , beta_peak=beta_comp)
    ...
    #         # # data poly
    #         # y_poly=y_data_win[ind_poly]
    #         # mean_poly=mean_level_win[ind_poly]
    ...
    #                     # This is not enough! we need something stronger here                    
    ...
    #                     while False:
    #                         # plt.plot(ind_poly,mean_poly)
    #                         # plt.plot(ind_poly,y_poly)
    #                         # plt.plot(ind_poly,y_poly-mean_poly)
    ...
    #         #
    #         if range_p.size==0:
    #             print('Storing Window',i,'Th',j,'No peak detected')
    #             fitting_data[i,j]=dict( idx_lr_poly = idx_lr_poly, range_peak = 0 )
    #         else:
    #             print('Storing Window',i,'Th',j,'Range',range_p[:,0],'--',range_p[:,1])
    #             fitting_data[i,j]=dict( idx_lr_poly = idx_lr_poly , mse_peak = mse_comp , range_peak = range_comp , beta_peak=beta_comp)
...
def recursive_merge_up(x_sub_win, y_sub_win, mean_level_sub, thr_sub, mse_up, range_up, beta_up, fit_fun, init_fit_fun):
        ...
        #if y_up_sub.size==0:
        if y_up_sub[np.where(y_up_sub>0)].size==0:
            return None
        ...
        #---------------------------------------------------
        #   MERGING INTERVALS -> NOT FOR NOW
        #---------------------------------------------------
        
        # merge intervals                       
        # peak_l_r_win = np.array(recursive_merge(peak_l_r_win.tolist())).astype(int)
        
        # remove the short intervars 
        if peak_l_r_win.size>0:
            peak_l_r_win =np.array(peak_l_r_win[np.where(peak_l_r_win[:,1]-peak_l_r_win[:,0]>6)[0],:])
        ...
        peak_l_r_win=peak_l_r_win.astype(int)
        ...
        if idx_lr_comp.size>0:
            ...
            for k in range(idx_lr_comp.shape[0]):
                ...
                # win_len=right-left
                # idx_comp = range( max([0,int(left-win_len/2)]),min([x_data_temp.shape[0],int(right+win_len/2)]))
                       
                #idx_comp = range(idx_lr_comp[k,0],idx_lr_comp[k,1])
                
                idx_comp_lr=find_mul_peak_sym_supp(idx_lr_comp[k],y_sub_win,mean_level_sub,0.85)
                
                if  idx_comp_lr==[0, y_sub_win.shape[0]-1]:
                    return None
                else:
                    idx_comp = range(idx_comp_lr[0],idx_comp_lr[1]+1)
                                                            
                    # print('Now processing the range',x_sub_win[idx_lr_comp[k,0]],'--',x_sub_win[idx_lr_comp[k,1]-1])

...
dim_s = 100
#<==========================================================================
def interpolate_and_find_peaks(frequency_vector, data_mean, data_mean_smoothed, window_param, degree_param):
    # <----------------------------------------Smoothe the mean of all data-----------------------------------------
    for i in range(0, data_mean.shape[0]):
        data_mean_smoothed[i] = sci.savgol_filter(data_mean[i], window_param, degree_param)
    # <=============================================================================================================
    ...
    for main_index in range(0, data_mean.shape[0]):
        for index, (value, value_error, value_error_abs) in enumerate(
                zip(data_mean[main_index], errors[main_index], errors_abs[main_index])):
        ...
        for index, peak in enumerate(islice(temp_list[main_index], len(temp_list[main_index]) - 1)):
            if (peak.index_pos - temp_list[main_index][index + 1].index_pos) == -1 and peak.value > \
                    temp_list[main_index][index + 1].value:
                if peak.value < temp_list[main_index][index - 1].value:
                    pass
                else:
                    peaks_list[main_index].append(peak)

        for index, (value_peak, frequency) in enumerate(zip(peaks_list[main_index], frequency_vector)):
            peak_frequencies[main_index].append(frequency_vector[value_peak.index_pos])
            peak_values[main_index].append(value_peak.value)

    return data_mean, peak_frequencies, peak_values
...
for i in range(6):
    ...
    plt.figure('{} {}'.format('raw data and peaks', i))
    ...
    plt.plot(peak_frequencies[index_ace], peak_values[index_ace] / max(data_mean[index_ace]), '*',
             label=peak_labels[index_ace])
...
# plt.plot(peak_frequencies[index_ace], peak_values[index_ace] / max(data_mean[index_ace]), '*',
#          label=peak_labels[index_ace])
# plt.plot(peak_frequencies[index_mg], peak_values[index_mg] / max(data_mean[index_mg]), '*',
#          label=peak_labels[index_mg])
# plt.plot(f_sup, data_mean[index_mix] / max(data_mean[index_mix]), '-', label=labels[index_mix])
# plt.plot(peak_frequencies[index_mix], peak_values[index_mix] / max(data_mean[index_mix]), '*',
#          label=peak_labels[index_mix])

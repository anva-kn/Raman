...
import shelve
# only store the data, the frequency vector, some constants and that's it

data=np.zeros([6,len_temp-1,dim])

data[0]=data_ace_temp[:len_temp-1]
data[1]=data_mg_temp[:len_temp-1]
data[2]=data_mix1_1_temp[:len_temp-1]
data[3]=data_mix1_2_temp[:len_temp-1]
data[4]=data_mix2_1_temp[:len_temp-1]
data[5]=data_mix2_2_temp[:len_temp-1]

#plt.plot(np.mean(data,axis=2).transpose())
#
